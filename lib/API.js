...
class API extends APIBase {
    ...
    async aliases() {
        let resp = await this.GET(
            `/api/v1/label/aliases`,
        );
        return resp.data;
    }

    /**
     * Create a standard representation of a label
     *
     * See: https://quizsage.org/docs/material_labels.md#canoicalize
     */
    async canonicalize_label(label) {
        let resp = await this.GET(
            `/api/v1/label/canonicalize${render_query_string({ label })}`,
        );
        return resp.data;
    }

    /**
     * Expand out a label into pure reference/ranges and canonicalize
     *
     * See: https://quizsage.org/docs/material_labels.md#descriptionize
     */
    async descriptionize_label(label) {
        let resp = await this.GET(
            `/api/v1/label/descriptionize${render_query_string({ label })}`,
        );
        return resp.data;
    }

    /**
     * Convert a label into json representation
     *
     * Inverse of format
     */
    async parse_label(label) {
        let resp = await this.GET(
            `/api/v1/label/parse${render_query_string({ label })}`,
        );
        return resp.data;
    }
    /**
     * Convert a json representation of a label into is descriptionized representation
     *
     * Inverse of parse
     */
    async format_label(obj) {
        let resp = await this.POST(
            `/api/v1/label/format`,
            obj
        );
